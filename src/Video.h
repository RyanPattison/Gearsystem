...
extern GS_Color *GG_Color_Table;
extern GS_Color SMS_Color_Table[4 * 4 * 4];

inline GS_Color Video::ConvertTo8BitColor(int palette_color)
{
    if (m_bGameGear) {
        u16 c = (m_pVdpCRAM[palette_color << 1]) | (m_pVdpCRAM[(palette_color << 1) + 1] << 8);
        return GG_Color_Table[c & 0xFFF];
    } else {
        return SMS_Color_Table[m_pVdpCRAM[palette_color] & 0x3F];
    }
}
